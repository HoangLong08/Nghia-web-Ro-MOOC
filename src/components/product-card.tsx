    ...
    <article className="group flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white transition-shadow hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <h3 className="mt-2 text-base font-semibold leading-snug text-slate-900">
          <Link href={`/san-pham/${product.slug}`} className="hover:text-amber-600">
            {product.name}
          </Link>
        </h3>
        ...
        <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 text-sm text-slate-600">
          <div>
            <dt className="inline text-slate-400">Số trục: </dt>
            <dd className="inline font-medium text-slate-700">{product.axles}</dd>
          </div>
          <div>
            <dt className="inline text-slate-400">Tải: </dt>
            <dd className="inline font-medium text-slate-700">{product.payload} tấn</dd>
          </div>
        </dl>
