...
export const metadata: Metadata = {
  title: "Sản phẩm",
  description:
    "Danh sách sơ mi rơ moóc sàn, xương, lửng, ben, bồn xi măng và bồn xăng dầu đang có tại bãi.",
};
...
const sorts = [
  ...
  { key: "tai-trong", label: "Tải trọng lớn nhất" },
] as const;
...
function sortProducts(list: Product[], key: string): Product[] {
  ...
  switch (key) {
    ...
    case "tai-trong":
      return sorted.sort((a, b) => b.payload - a.payload);
    default:
      return sorted;
  }
}
...
export default async function ProductsPage({
  ...
  const categorySlug = first(params["danh-muc"]);
  const sortKey = first(params["sap-xep"]) ?? "moi-nhat";
  ...
  const buildHref = (next: { category?: string; sort?: string }) => {
    ...
    return { pathname: "/san-pham", query };
  };
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <h1 className="mt-3 text-3xl font-bold text-slate-900">
        {activeCategory ? activeCategory.name : "Tất cả sơ mi rơ moóc"}
      </h1>
      <p className="mt-2 max-w-3xl text-slate-600">
        {activeCategory
          ? activeCategory.description
          : "Toàn bộ cấu hình đang có tại bãi. Giá đã gồm VAT, chưa gồm phí đăng ký và đăng kiểm."}
      </p>
